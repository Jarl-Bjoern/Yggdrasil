...
function header() {
        ...
        if [ "$1" = "category" ]; then
				...
				echo -e "${CYAN}|${NOCOLOR}  [${NOCOLOR}0${NOCOLOR}] ${NOCOLOR}skip${NOCOLOR}          :   skip settings                          ${CYAN}|${NOCOLOR}"
        elif [ "$1" = "task" ]; then
...
if [[ "$Switch_UPDATES" == true ]]; then
	mkdir -p /etc/yggdrasil ; chown root: /etc/yggdrasil
        if [[ "$Switch_CRON" == true ]]; then
				...
				sudo python3 "$FULL_PATH/Resources/Python/filter.py" "/usr/lib/systemd/system" "$OPT_Path" "systemd_stop"
				...
				sudo systemctl enable Yggdrasil_Service_Stop.service &>/dev/null
        fi
