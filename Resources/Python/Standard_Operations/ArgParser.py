...
"""
  ...
  if (osname == 'nt'):
      ...
      optional.add_argument('-rCf','--read-config-file', type=str, default=f"{dirname(realpath(__file__)).replace('Resources/Python/Standard_Operations', 'Automation/Install.cfg')}", help=Colors.GREEN+'UNDER CONSTRUCTION.\n\n'+Colors.RESET+f"Default: {dirname(realpath(__file__)).replace('Resources/Python/Standard_Operations', 'Automation/Install.cfg')}\n\n"+Colors.BLUE+'-------------------------------------------------------------------------------------'+Colors.RESET)
