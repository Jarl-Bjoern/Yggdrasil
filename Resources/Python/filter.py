...
alias yggdrasil-vnc='if [[ $(netstat -tnap | grep "x11vnc" | cut -d "/" -f1 | rev | cut -d " " -f1 | rev | sort -u) ]]; then for i in $(netstat -tnap | grep "x11vnc" | cut -d "/" -f1 | rev | cut -d " " -f1 | rev | sort -u); do kill $i; done; fi; if [[ $(netstat -tnap | grep -v "tcp6" | awk "{print $4}" | cut -d ":" -f2 | grep "8081") ]]; then kill $(netstat -tnap | grep -v "tcp6" | grep "0.0.0.0:8081" | awk "{print $7}" | cut -d "/" -f1); fi; sudo x11vnc -storepasswd ; sudo x11vnc -display :0 -autoport -bg -localhost -rfbauth ~/.vnc/passwd -xkb -ncache -ncache_cr -quiet & ; /usr/share/novnc/utils/novnc_proxy --listen 8081 --vnc localhost:5900 --ssl-only --key /etc/apache2/ssl/pentest-key.pem --cert /etc/apache2/ssl/pentest-cert.pem'
. "$HOME/.cargo/env"
"""
